...
class CN_Facilities_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$options = array(
			'description' => __( 'Business Facilities', 'Connections_Degrees' )
		);

		parent::__construct(
			'cn_facilities',
			'Connections : ' . __( 'Business Facilities', 'Connections_Degrees' ),
			$options
		);
	}
	...
	public function form( $instance ) {
		...
		cnHTML::text(
			array(
				...
				'label'  => __( 'Title:', 'Connections_Degrees' ),
				'before' => '<p>',
				'after'  => '</p>',
			),
			$title
	...
	public function widget( $args, $option ) {
		...
		if ( get_query_var( 'cn-entry-slug' ) ) {
			...
			$title = strlen( $option['title'] ) > 0 ? $option['title'] : __( 'Facilities', 'Connections_Degrees' );
			...
			Connections_Degrees::block( $entry, $atts, NULL );
